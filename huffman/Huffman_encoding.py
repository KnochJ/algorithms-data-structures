...
def huffmanEncode(inputString):
    ...
    numChars = len(chDiction) 
    #minQ = min_heapify(chQueue, 0)
    #print(minQ)
    for i in range(0, numChars-1):
        ...
        #print("insert super node into minQ")
        #print(minQ)
    #return heap_extract_min(minQ)
    
    #print(super_node)
    
    # Encode from slides only does one letter,
    # so we need to do it for each ch in inputString.
    # We need to add all of the binary together.
    fullEncode = ""
    for ch in inputString:
        #code gets the binary for that specific path
        code = ""
        #node = root, here super_node is the root
        node = super_node
        while(ch != node.ch):
...
def min_heapify(Q,i):
    ...
    # added a return of Q otherwise we get a NoneType
    return Q
