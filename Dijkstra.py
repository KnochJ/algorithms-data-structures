...
def hyperspace_jump_route(star_list,jump_times):
    ...
    goal_node = star_list[1]

    #while star_list is not empty
    #   extract the minimum in the starlist, start is red since 0
    star_list[0].set_dist(0)
    while len(star_list) > 0:
        #print(star_list)
        min_star = star_list[0]
        star_list.pop(0)
        #print("min_star is :")
        #print(min_star)
        #print("min_star adjacency is: ")
        #print(min_star.adj)
        #   for each star adjacent to this star
        #   figure out the distances and sort them, then back to extract
        #print("----------------------------")
        for star in min_star.adj:
            if star.get_dist() > min_star.get_dist() + jump_times[min_star.num][star.num]: #if v.dist > u.dist + weights(u,v)
                star.set_dist(min_star.get_dist() + jump_times[min_star.num][star.num])
                # here we need to order the list
                star_list.sort(key=lambda x: x.get_dist(), reverse=False)
                '''
                temp_node = star
                if len(star_list) > 0 :
                    index = star_list.index(star)
                    del star_list(index)
                    star_list.insert(0,temp_node)
                '''
                star.prev = min_star          #v.prev = u
    while goal_node.get_dist() != 0:
        route_queue.append(goal_node)
