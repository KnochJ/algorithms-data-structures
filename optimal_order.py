...
def optimizeNuggets(priceList,numNuggets):
    ...
    #create rev and initial split list
    rev = [0]*(numNuggets+1)
    s = [0]*(numNuggets+1)
    rev[0] = 0
    for j in range(1,numNuggets+1):
        best = float("inf") #sentinel
        for i in range(1,j+1):
            # We need to minimize cost so we must make
            #  slight adjustment to the books algorithm:
            #  best = inf and best >
    ...
    # create our final split which will return the optimal split
    split = []
    while numNuggets > 0:
            # add the value to the end of split list
            split.append(s[numNuggets])
            #print(s[numNuggets])  #used to see what values are optimal
            numNuggets = numNuggets - s[numNuggets]

    # return our optimal split
    return split
